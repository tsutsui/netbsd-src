...
/* lifted from mips */
static int
db_setf_regs(const struct db_variable      *vp,
	db_expr_t		*valuep,
	int			op)		/* read/write */
{
#if 0 /* XXX TKM */
	int   *regp = (int *) ((char *) regs + (int) (vp->valuep));
#else
	db_regs_t* regs = (vp->modif[0] == 'u' ? &curlwp->l_md.md_tf->tf_regs : DDB_REGS);
	int   *regp = (int *) ((char *) regs + (int) (vp->valuep));
#endif
 ...
 */
static vaddr_t
stack_decode(db_addr_t addr, vaddr_t *stack, void (*pr)(const char *, ...))
{
	db_sym_t proc;
	...
	if (proc != DB_SYM_NULL) {
		...
		for (check_addr = addr; instructions_to_search-- != 0;
			...
			if (ST_R30_R31_IMM(inst)) {
				DPRINTF(("{st r30 found at 0x%08lx}\n",
				    check_addr));
				str30_addr = (vaddr_t)check_addr;
			} else if (SUBU_R31_R31_IMM(inst)) {
				DPRINTF(("{subu r31 found at 0x%08lx}\n",
				    check_addr));
	...
	DPRINTF(("{start of function at 0x%08lx}\n", function_addr));
	/*
	...
	*stack += IMM16VAL(inst);
	...
	if (function_addr == (vaddr_t)&panic) /* XXX others? */ {
		...
		while (instructions_to_search-- != 0) {
			if (!db_trace_get_val(check_addr, &inst))
				break;
			if (SUBU_R31_R31_IMM(inst)) {
				DPRINTF(("{variadic subu r31 found at 0x%08lx}\n",
				    check_addr));
				*stack += IMM16VAL(inst);
			}
			check_addr += 4;
	...
	r31 = *stack;
	DPRINTF(("{entry r31 would be 0x%08x}\n", r31));
	for (instructions_to_search = (addr - check_addr) / sizeof(uint32_t);
		...
		if (SUBU_R31_R31_IMM(inst)) {
			r31 -= IMM16VAL(inst);
			DPRINTF(("{adjust r31 to 0x%08x at 0x%08lx}\n",
			    r31, check_addr));
		}
		...
		if ((flags & STORE) && s1 == 31 /*stack pointer*/) {
			...
			if (!have_local_reg(d)) {
				...
				DPRINTF(("{r%d saved at 0x%08lx to 0x%08x}\n",
...
static void
db_stack_trace_cmd2(db_regs_t *regs, void (*pr)(const char *, ...))
{
	vaddr_t stack;
	u_int depth=1;
	vaddr_t where;
